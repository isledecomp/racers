...
- **Start loop recovery from the natural indexed source.** When the original has a walking pointer, a one-based induction variable, pointer post-increments, or a separately incremented derived byte/element offset, treat those as likely MSVC 6.0 strength reduction first, not source locals. Begin with the simple human loop: `for (i = 0; i < N; i++)` over the real collection, with indexed assignments like `items[i] = base + i * stride`, derived values like `(i * 2) + 1`, and adjacent indices `i - 1` / `i + 1`. Only introduce explicit walking pointers or manual offset accumulators after the indexed form fails; reifying every observed register/stack local in source can make the match worse.
