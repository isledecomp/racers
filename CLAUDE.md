...
- **Reproducing the original's partial ICF folding with `#pragma code_seg` (permanent, evidence-backed).** The original binaries fold identical trivial bodies only within certain groups: byte-identical functions coexist at 2-3 separate addresses per byte pattern (e.g. `xor eax,eax; ret 0xc` at both 0x466090 and 0x473490; verify with `tools/fold_census.py <orig> <recomp>`). A uniform `/OPT:REF,ICF` link folds each pattern image-wide instead. Wrap each member of a split group in a named section (`#pragma code_seg(".text$group")` ... `#pragma code_seg()`): same name = members still fold together, different names = groups stay apart. Suffix names do not perturb placement under link 6.00. Symptom of a missing pragma: reccmp "Failed to find function symbol" / GONE entries + vtable slots pointing at a wrong fold survivor. Do NOT use for functions the original itself folds into the image-wide group — those stay `FOLDED` without pragmas.
