#ifndef GOLWDBDATABASE_H
#define GOLWDBDATABASE_H
...
class GolFileParser;
class GolWdbEntity;
class GolWdbModelEntity;
class GolWdbBoundedEntity;
class GolWdbSkinnedEntity;
class GolWdbCollidableEntity;
...
public:
	...
	class WdbTxtParser : public GolTxtParser {
		...
		// GolWdbDatabase::WdbTxtParser::`scalar deleting destructor'

		// SYNTHETIC: GOLDP 0x1001b000 FOLDED
		// GolWdbDatabase::WdbTxtParser::~WdbTxtParser
	};
	...
	virtual ~GolWdbDatabase();                                                                         // vtable+0x10
	...
	virtual GolTdbTextureList* VTable0x2c(LegoU32 p_index) const = 0;                                  // vtable+0x2c
	virtual GolMdbMaterialList* VTable0x30(LegoU32 p_index) const = 0;                                 // vtable+0x30
	virtual CmbModelPart0x34* VTable0x34(LegoU32 p_index) const = 0;                                   // vtable+0x34
	virtual GolGdbModelBase* VTable0x38(LegoU32 p_index) const = 0;                                    // vtable+0x38
	virtual GolGdbMaterialList* VTable0x3c(LegoU32 p_index) const = 0;                                 // vtable+0x3c
	virtual GolSceneNode* VTable0x40(LegoU32 p_index) const = 0;                                       // vtable+0x40
	virtual GolBdbBoundingShape* VTable0x44(LegoU32 p_index) const = 0;                                // vtable+0x44
	virtual GolWdbEntity* VTable0x48(LegoU32 p_index) const = 0;                                       // vtable+0x48
	...
	// GolWdbDatabase::`scalar deleting destructor'
	...
	GolWdbSkinnedEntity* GetUnk0xa0() const { return m_unk0xa0; }
	...
	GolWdbSkinnedEntity* GetUnk0xc0Name(const LegoChar* p_name) const
	{
		return static_cast<GolWdbSkinnedEntity*>(m_unk0xc0.GetName(p_name));
	}
	GolWdbModelEntity* FindUnk0xb4(const LegoChar* p_name) const
	{
		return m_unk0xb4.GetNameEntries() ? static_cast<GolWdbModelEntity*>(m_unk0xb4.GetName(p_name)) : NULL;
	}
	GolWdbSkinnedEntity* FindUnk0xc0(const LegoChar* p_name) const
	{
	...
	GolWdbCollidableEntity* FindUnk0xcc(const LegoChar* p_name) const
	{
		return m_unk0xcc.GetNameEntries() ? static_cast<GolWdbCollidableEntity*>(m_unk0xcc.GetName(p_name)) : NULL;
	}
...
protected:
	...
	void FUN_1002e250(GolWdbBillboard* p_billboard, WdbBillboardSprite0x38* p_sprite);
	void FUN_1002e640();
	...
	GolBvbBoundingVolume* m_unk0x98;           // 0x98
	GolWdbModelEntity* m_unk0x9c;              // 0x9c
	GolWdbSkinnedEntity* m_unk0xa0;            // 0xa0
	GolWdbCollidableEntity* m_unk0xa4;         // 0xa4
	GolWdbBoundedEntity* m_unk0xa8;            // 0xa8
