...
DECOMP_SIZE_ASSERT(GolFdbFontList::FdbTxtParser, 0x1fc)
DECOMP_SIZE_ASSERT(GolFdbFontList::FontParseStyle, 0x0c)
DECOMP_SIZE_ASSERT(GolFdbFontList, 0x28)

// FUNCTION: GOLDP 0x1001d830
GolFdbFontList::GolFdbFontList()
{
	m_renderer = NULL;
	m_numItems = 0;
	m_unk0x24 = 0;
...
void GolFdbFontList::VTable0x20(GolD3DRenderDevice* p_renderer, const LegoChar* p_fileName, LegoBool32 p_binary)
{
	if (m_numItems > 0) {
		Clear();
	}
	...
	for (LegoU32 i = 0; i < m_numItems; i++) {
		...
		while (token != GolFileParser::e_rightCurly) {
			...
			case GolFileParser::e_unknown0x2b: {
				...
				GolFdbFontList::ReadFontCharList(parser, chars, &count);
				m_unk0x20[i] = count;
				VTable0x10(i);
...
void __stdcall GolFdbFontList::ReadFontCharList(GolFileParser* p_parser, undefined2* p_chars, LegoU16* p_count)
{
	*p_count = 0;
...
void GolFdbFontList::ReleaseFontSurfaces()
{
	// empty
}

// FUNCTION: GOLDP 0x10029920 FOLDED
void GolFdbFontList::RefreshFontSurfaces()
{
	// empty
}
