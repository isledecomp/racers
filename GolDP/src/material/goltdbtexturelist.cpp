...
DECOMP_SIZE_ASSERT(GolTdbTextureList, 0x20)
DECOMP_SIZE_ASSERT(GolTdbTextureList::TdbTxtParser, 0x1fc)
...
GolTdbTextureList::GolTdbTextureList()
{
	m_renderer = NULL;
	m_next = NULL;
	m_unk0x14 = 0;
...
void GolTdbTextureList::VTable0x24(GolD3DRenderDevice* p_renderer, const LegoChar* p_fileName, LegoBool32 p_binary)
{
	if (m_renderer != NULL) {
		Clear();
	}
...
void GolTdbTextureList::LoadTextures()
{
	if (m_unk0x14 != NULL) {
		for (LegoU32 i = 0; i < m_numItems; i++) {
...
void GolTdbTextureList::VTable0x20(GolD3DRenderDevice* p_renderer, MagentaRibbonSource0x4* p_source, LegoU32 p_capacity)
{
	if (m_renderer != NULL) {
		Clear();
	}
...
void GolTdbTextureList::VTable0x1c(GolD3DRenderDevice* p_renderer, LegoU32 p_capacity)
{
	if (m_renderer != NULL) {
		Clear();
	}
...
void GolTdbTextureList::VTable0x0c()
{
	// empty
}

// FUNCTION: GOLDP 0x10029920 FOLDED
void GolTdbTextureList::VTable0x10()
{
	// empty
}
