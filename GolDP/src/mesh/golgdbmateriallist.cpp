...
DECOMP_SIZE_ASSERT(GolGdbMaterialList, 0x0c)

// FUNCTION: GOLDP 0x10025de0 FOLDED
GolGdbMaterialList::GolGdbMaterialList()
{
	m_renderer = NULL;
	m_count = 0;
	m_entries = NULL;
...
void GolGdbMaterialList::FUN_10025df0(GolRenderDevice* p_renderer, undefined4 p_arg2)
{
	if (m_renderer != NULL) {
		Destroy();
	}
...
void GolGdbMaterialList::FUN_10025e60(GolRenderDevice* p_renderer, const LegoChar* p_fileName, LegoBool32 p_binary)
{
	if (m_renderer != NULL) {
		Destroy();
	}
	...
	if (p_binary) {
		...
		parser = new GolGdbModelBase::GdbTxtParser;
		if (parser == NULL) {
			GOL_FATALERROR(c_golErrorOutOfMemory);
		}
	}
...
void GolGdbMaterialList::FUN_10025f90(GolRenderDevice* p_renderer, GolFileParser& p_parser)
{
	LegoU32 i;
...
// FUNCTION: GOLDP 0x100260d0 FOLDED
void GolGdbMaterialList::Destroy()
{
	if (m_entries != NULL) {
		delete[] m_entries;
		m_entries = NULL;
