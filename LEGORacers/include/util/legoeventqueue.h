...
// SIZE 0x2c
class LegoEventQueue::ProximityQueue : public LegoEventQueue {
public:
	ProximityQueue();
	LegoS32 AddEvent(Event* p_event) override; // vtable+0x00

	// SYNTHETIC: LEGORACERS 0x0042fca0
	// LegoEventQueue::ProximityQueue::`scalar deleting destructor'
...
// SIZE 0x48
class LegoEventQueue::CollisionQueue : public ProximityQueue {
public:
	CollisionQueue();
	LegoS32 AddEvent(Event* p_event) override;               // vtable+0x00
	void Destroy() override;                                 // vtable+0x0c
