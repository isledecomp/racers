...
void DirectInputDevice::FUN_0044faa0()
{
	...
	m_devType = LOBYTE(deviceInfo.dwDevType);
	m_devSubType = HIBYTE(deviceInfo.dwDevType);
...
LegoBool32 DirectInputDevice::VTable0x58()
{
	...
	DIPERIODIC periodicParams= {
		/*dwMagnitude*/ 3000, // range = [0, 10000]
		/*lOffset*/ 	0,
		/*dwPhase*/ 	0,
		/*dwPeriod*/ 	50000, // unit = microseconds
	};
	DIEFFECT effectParams = {
		/* dwSize */					sizeof(effectParams),
		/* dwFlags */					DIEFF_POLAR | DIEFF_OBJECTOFFSETS,
		/* dwDuration */				INFINITE,
		/* dwSamplePeriod */			0,
		/* dwGain */					10000, // range = [0, 10000]
		/* dwTriggerButton */			DIEB_NOTRIGGER,
		/* dwTriggerRepeatInterval */	0,
		/* cAxes */						sizeOfArray(axes),
		/* rgdwAxes */					axes,
		/* rglDirection */				direction,
		/* lpEnvelope */				NULL,
		/* cbTypeSpecificParams */		sizeof(periodicParams),
		/* lpvTypeSpecificParams */		&periodicParams,
	};
#else
	DIPERIODIC periodicParams;
	DIEFFECT effectParams;
...
#endif
	if (FAILED(m_device->CreateEffect(GUID_Sine, &effectParams, &m_effect, NULL))) {
		m_effect = NULL;
	}
	return FALSE;
}
