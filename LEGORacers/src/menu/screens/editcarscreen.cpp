#include "menu/screens/editcarscreen.h"

DECOMP_SIZE_ASSERT(EditCarScreen, 0x36c4)

// STUB: LEGORACERS 0x0047bc50
EditCarScreen::EditCarScreen()
{
	STUB(0x0047bc50);
}

// STUB: LEGORACERS 0x0047be00
EditCarScreen::~EditCarScreen()
{
	STUB(0x0047be00);
}

// STUB: LEGORACERS 0x0047bf70
void EditCarScreen::Reset()
{
	STUB(0x0047bf70);
}

// STUB: LEGORACERS 0x0047c0e0
void EditCarScreen::VTable0x4c()
{
	STUB(0x0047c0e0);
}

// STUB: LEGORACERS 0x0047c1d0
void EditCarScreen::VTable0x80()
{
	STUB(0x0047c1d0);
}

// STUB: LEGORACERS 0x0047c210
LegoBool32 EditCarScreen::VTable0x8c(MenuToolContext0x4bc8*, MenuToolCreateParams0x30*)
{
	STUB(0x0047c210);
	return FALSE;
}

// STUB: LEGORACERS 0x0047c2c0
LegoBool32 EditCarScreen::Destroy()
{
	STUB(0x0047c2c0);
	return FALSE;
}

// STUB: LEGORACERS 0x0047c980
void EditCarScreen::VTable0x38(ObscureVantage0x58*)
{
	STUB(0x0047c980);
}

// STUB: LEGORACERS 0x0047cbc0
void EditCarScreen::VTable0x44(ObscureVantage0x58*)
{
	STUB(0x0047cbc0);
}

// STUB: LEGORACERS 0x0047ccf0
void EditCarScreen::VTable0x84()
{
	STUB(0x0047ccf0);
}
