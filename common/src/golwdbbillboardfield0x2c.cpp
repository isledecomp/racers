...
GolWdbBillboard::Field0x2c::Field0x2c()
{
	m_renderer = NULL;
	m_count = 0;
	m_entries = NULL;
...
void GolWdbBillboard::Field0x2c::Initialize(GolRenderDevice* p_renderer, LegoU32 p_count)
{
	if (m_renderer != NULL) {
		Clear();
	}
...
// FUNCTION: GOLDP 0x100260d0 FOLDED
void GolWdbBillboard::Field0x2c::Clear()
{
	if (m_entries != NULL) {
		delete[] m_entries;
		m_entries = NULL;
